...
export class BondingCurveAccount {
  ...
  getBuyPricePredicted(amount: bigint, vSolReserves: bigint, vTokenReserves: bigint, rTokenReserves: bigint): bigint {
    if (this.complete) {
      throw new Error("Curve is complete");
    }
    ...
    let n = vSolReserves * vTokenReserves;
    let i = vSolReserves + amount;
    let r = n / i + 1n;
    let s = vTokenReserves - r;
    return s < rTokenReserves ? s : rTokenReserves;
  }

  getSellPricePredicted(amount: bigint, feeBasisPoints: bigint, vSolReserves: bigint, vTokenReserves: bigint): bigint {
    if (this.complete) {
      throw new Error("Curve is complete");
    }
    ...
    let n =
      (amount * vSolReserves) / (vTokenReserves + amount);
